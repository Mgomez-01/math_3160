...
\begin{align*}
  &\int_{C_i} z^{\frac{1}{2}} dz\\
  z^{\frac{1}{2}} = e^{\frac{1}{2} \log{(z)}} &= e^{\frac{(\ln{|z|} + i \text{arg}(z))}{2}} = \sqrt{|z|}e^{\frac{i \text{arg}(z)}{2}} \\
  f(z) &= \sqrt{|z|}e^{\frac{i \text{arg}(z)}{2}}\\
  z_1(t) &= 3e^{i\pi (1-t)} \quad \quad 0 \leq t < 1
\end{align*}
This path takes us above in a circular motion clockwise from $-3$ to $3$. to get the path on the lower side, we can remove the negative in the exponential:

Path $C_2$:
\begin{align*}
  &\int_{C_2} z^{\frac{1}{2}} dz\\
  z^{\frac{1}{2}} = e^{\frac{1}{2} \log{(z)}} &= e^{\frac{(\ln{|z|} + i \text{arg}(z))}{2}} = \sqrt{|z|}e^{\frac{i \text{arg}(z)}{2}} \\
  f(z) &= \sqrt{|z|}e^{\frac{i \text{arg}(z)}{2}}\\
  z_2(t) &= 3e^{i\pi t} \quad \quad 1 \leq t < 2
\end{align*}
